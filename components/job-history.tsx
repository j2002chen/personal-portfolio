...
const experiences = [
  ...
  return (
    <div className={`container mx-auto px-4 ${styles.container}`}>
      <h2 className="text-3xl font-bold mb-7" style={{ fontFamily: 'var(--font-mono)', paddingBottom: '1rem' }}>Work Experience</h2>
      {experiences.map((exp, index) => (
        <div 
          key={index} 
          className={`flex flex-col py-6 border-b border-gray-700 ${styles.experience}`} 
        ...
        >
          ...
          {expandedIndex === index && (
            <div className={`experience-details mt-2 text-sm ${styles.detailsSection}`}>
              <p>{exp.details}</p> 
            </div>
          )}
        </div>
